...
const float ProbenderInGameData::MIN_JUMP_SPEED = 6.0f;
const float ProbenderInGameData::MAX_JUMP_SPEED = 12.0f;
...
void ProbenderInGameData::FillFromProbenderData(const ProbenderData& data)
{
	...
	SkillsBonus.JumpHeight = CalculateJumpSpeed(data.Skills.Jump);
}
...
float ProbenderInGameData::CalculateJumpSpeed(const short jumpSkill)
{
	//Give 1% of the speed for a 0 skill
	if(jumpSkill == 0)
		return MIN_JUMP_SPEED;
	...
	//0.5 * 0.65
	//0.325f is the amount of time it should take to complete the dodge
	return ((MAX_JUMP_SPEED - MIN_JUMP_SPEED) * ((float)jumpSkill / ProbenderGeneralSkills::MAX_SKILL_POINTS_ALLOWED) + MIN_JUMP_SPEED);
}
